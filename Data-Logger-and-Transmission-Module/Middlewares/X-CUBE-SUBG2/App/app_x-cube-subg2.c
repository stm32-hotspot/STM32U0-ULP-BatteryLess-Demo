...
uint8_t TxLengthTest = 4;
uint8_t aTransmitBufferTest[4] = {1,2,3,4};
...
//// Private function
void send_Data(uint8_t *pTxBuff, uint8_t cTxlen);
...
//  HAL_Delay(1000);
  ...
  ////
//  S2LPGpioSetLevel(S2LP_GPIO_3, LOW);
  ////
...
//  TxFrameBuff[4] = xTxFrame->DataLen;
  ...
  {
    TxFrameBuff[xIndex+1] =  xTxFrame->DataBuff[xIndex];
  }
...
void MCU_Enter_SleepMode(void)
{
  ...
  BSP_JOY_Init(JOY_KEY, JOY_MODE_EXTI, JOY_SEL);
// BSP_PB_Init(BUTTON_KEY, BUTTON_MODE_EXTI);
