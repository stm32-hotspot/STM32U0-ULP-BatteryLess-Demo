/**
  ******************************************************************************
  * @file    stm32u083c_discovery_glass_lcd.c
  * @author  MCD Application Team
  * @brief   This file provides a set of functions needed to manage the
  *          LCD Glass driver for the STM32U083C-DK board.
  ******************************************************************************
  * @attention
  *
...
/** @defgroup STM32U083C_DK_GLASS_LCD STM32U083C-DK GLASS LCD
...
/** @defgroup STM32U083C_DK_GLASS_LCD_Defines LCD Private Defines
  * @{
  */
...
extern uint16_t calculated_illuminance;
extern uint8_t Is_RF_Module_Connected;
uint8_t rolling_bar = 0;
uint8_t no_sending_cnt = 0;
...
/** @defgroup STM32U083C_DK_LCD_Private_Functions Private Functions
  * @{
  */
...
/** @addtogroup STM32U083C_DK_LCD_Exported_Functions LCD Exported Functions
  * @{
  */
...
//  LCDHandle.Init.HighDrive        = LCD_HIGHDRIVE_ENABLE;
  ...
  LCDHandle.Init.DeadTime         = LCD_DEADTIME_0;
  LCDHandle.Init.PulseOnDuration  = LCD_PULSEONDURATION_1;
#elif CONFIG_LCD == 2
  LCDHandle.Init.Prescaler        = LCD_PRESCALER_4;
  LCDHandle.Init.HighDrive        = LCD_HIGHDRIVE_DISABLE;
  LCDHandle.Init.Contrast         = LCD_CONTRASTLEVEL_6;
  LCDHandle.Init.DeadTime         = LCD_DEADTIME_0;
  LCDHandle.Init.PulseOnDuration  = LCD_PULSEONDURATION_1;
#else
  LCDHandle.Init.Prescaler        = LCD_PRESCALER_4;
...
//#if LCD_BUFFER_ON > 0
  __HAL_LCD_VOLTAGE_BUFFER_ENABLE(&LCDHandle);
//#endif
...
void BSP_LCD_GLASS_DisplayBar(uint32_t BarId)
{
  ...
  while ((BarId) >> position)
  {
    ...
    switch (BarId & (1 << position))
    {
      ...
      case LCD_BAR_0:
        /* Set BAR0 */
        HAL_LCD_Write(&LCDHandle, LCD_BAR0_COM, ~(LCD_BAR0_SEG), LCD_BAR0_SEG);
        break;

        /* Bar 1 */
      case LCD_BAR_1:
        /* Set BAR1 */
        HAL_LCD_Write(&LCDHandle, LCD_BAR1_COM, ~(LCD_BAR1_SEG), LCD_BAR1_SEG);
        break;

        /* Bar 2 */
      case LCD_BAR_2:
        /* Set BAR2 */
        HAL_LCD_Write(&LCDHandle, LCD_BAR2_COM, ~(LCD_BAR2_SEG), LCD_BAR2_SEG);
        break;

        /* Bar 3 */
      case LCD_BAR_3:
        /* Set BAR3 */
        HAL_LCD_Write(&LCDHandle, LCD_BAR3_COM, ~(LCD_BAR3_SEG), LCD_BAR3_SEG);
        break;
...
void BSP_LCD_GLASS_ClearBar(uint32_t BarId)
{
  ...
  while ((BarId) >> position)
  {
    ...
    switch (BarId & (1 << position))
    {
      ...
      case LCD_BAR_0:
        /* Set BAR0 */
        HAL_LCD_Write(&LCDHandle, LCD_BAR0_COM, ~(LCD_BAR0_SEG) , 0);
        break;

        /* Bar 1 */
      case LCD_BAR_1:
        /* Set BAR1 */
        HAL_LCD_Write(&LCDHandle, LCD_BAR1_COM, ~(LCD_BAR1_SEG), 0);
        break;

        /* Bar 2 */
      case LCD_BAR_2:
        /* Set BAR2 */
        HAL_LCD_Write(&LCDHandle, LCD_BAR2_COM, ~(LCD_BAR2_SEG), 0);
        break;

        /* Bar 3 */
      case LCD_BAR_3:
        /* Set BAR3 */
        HAL_LCD_Write(&LCDHandle, LCD_BAR3_COM, ~(LCD_BAR3_SEG), 0);
        break;
...
void BSP_LCD_GLASS_DisplayStrDeci(uint16_t *ptr)
{
  ...
  ////////
  if(calculated_illuminance > THRESHOLD_LUX_NO_SENDING && Is_RF_Module_Connected > 0){
    
    if(rolling_bar == 0){
      /* Set BAR0 on */
    HAL_LCD_Write(&LCDHandle, LCD_BAR0_COM, ~(LCD_BAR0_SEG), LCD_BAR0_SEG);
    }else if(rolling_bar == 1){
    /* Set BAR1 on */
    HAL_LCD_Write(&LCDHandle, LCD_BAR1_COM, ~(LCD_BAR1_SEG), LCD_BAR1_SEG);
    }else if(rolling_bar == 2){
    /* Set BAR2 on */
    HAL_LCD_Write(&LCDHandle, LCD_BAR2_COM, ~(LCD_BAR2_SEG), LCD_BAR2_SEG);
    }else if(rolling_bar == 3){
    /* Set BAR3 on */
    HAL_LCD_Write(&LCDHandle, LCD_BAR3_COM, ~(LCD_BAR3_SEG), LCD_BAR3_SEG);
    rolling_bar = -1;
    }
...
/** @addtogroup STM32U083C_DK_LCD_Private_Functions LCD Private Functions
  * @{
  */
...
static void LCD_MspDeInit(LCD_HandleTypeDef *hlcd)
{
  ...
  /* Port E */
  gpiopin = LCD_GPIO_BANKE_PINS;
  HAL_GPIO_DeInit(GPIOE, gpiopin);
...
#else
      data = ((Digit[0] & 0x1) << LCD_SEG0_SHIFT) | (((Digit[0] & 0x2) >> 1) << LCD_SEG1_SHIFT)
             |(((Digit[0] & 0x8) >> 3) << LCD_SEG23_SHIFT);
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT1_COM0, LCD_DIGIT1_COM0_SEG_MASK, data); /* 1G 1B 1M 1E */

      data = (((Digit[0] & 0x4) >> 2) << LCD_SEG22_SHIFT) ;
       HAL_LCD_Write(&LCDHandle, LCD_DIGIT1_COM0_1, LCD_DIGIT1_COM0_1_SEG_MASK, data); /* 1G 1B 1M 1E */

      data = ((Digit[1] & 0x1) << LCD_SEG0_SHIFT) | (((Digit[1] & 0x2) >> 1) << LCD_SEG1_SHIFT)
             |(((Digit[1] & 0x8) >> 3) << LCD_SEG23_SHIFT);
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT1_COM1, LCD_DIGIT1_COM1_SEG_MASK, data); /* 1F 1A 1C 1D  */
      
      data =  (((Digit[1] & 0x4) >> 2) << LCD_SEG22_SHIFT) ;
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT1_COM1_1, LCD_DIGIT1_COM1_1_SEG_MASK, data); /* 1F 1A 1C 1D  */

      data = ((Digit[2] & 0x1) << LCD_SEG0_SHIFT) | (((Digit[2] & 0x2) >> 1) << LCD_SEG1_SHIFT)
             |  (((Digit[2] & 0x8) >> 3) << LCD_SEG23_SHIFT);
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT1_COM2, LCD_DIGIT1_COM2_SEG_MASK, data); /* 1Q 1K 1Col 1P  */
      
      data =  (((Digit[2] & 0x4) >> 2) << LCD_SEG22_SHIFT) ;
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT1_COM2_1, LCD_DIGIT1_COM2_1_SEG_MASK, data) ; /* 1Q 1K 1Col 1P  */

      data = ((Digit[3] & 0x1) << LCD_SEG0_SHIFT) | (((Digit[3] & 0x2) >> 1) << LCD_SEG1_SHIFT)
             |(((Digit[3] & 0x8) >> 3) << LCD_SEG23_SHIFT);
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT1_COM3, LCD_DIGIT1_COM3_SEG_MASK, data); /* 1H 1J 1DP 1N  */
         
      data =  (((Digit[3] & 0x4) >> 2) << LCD_SEG22_SHIFT) ;
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT1_COM3_1, LCD_DIGIT1_COM3_1_SEG_MASK, data); /* 1H 1J 1DP 1N  */
...
#else
      data = ((Digit[0] & 0x1) << LCD_SEG6_SHIFT) |(((Digit[0] & 0x2) >> 1) << LCD_SEG7_SHIFT) | (((Digit[0] & 0x4) >> 2) << LCD_SEG16_SHIFT) ;
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT4_COM0, LCD_DIGIT4_COM0_SEG_MASK, data); /* 1G 1B 1M 1E */
      data = (((Digit[0] & 0x8) >> 3) << LCD_SEG17_SHIFT);
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT4_COM0_1, LCD_DIGIT4_COM0_1_SEG_MASK, data); /* 1G 1B 1M 1E */

      data = ((Digit[1] & 0x1) << LCD_SEG6_SHIFT) |  (((Digit[1] & 0x2) >> 1) << LCD_SEG7_SHIFT) | (((Digit[1] & 0x4) >> 2) << LCD_SEG16_SHIFT);
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT4_COM1, LCD_DIGIT4_COM1_SEG_MASK, data); /* 1F 1A 1C 1D  */
      data =(((Digit[1] & 0x8) >> 3) << LCD_SEG17_SHIFT);
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT4_COM1_1, LCD_DIGIT4_COM1_1_SEG_MASK, data) ; /* 1F 1A 1C 1D  */

      data = ((Digit[2] & 0x1) << LCD_SEG6_SHIFT) |(((Digit[2] & 0x2) >> 1) << LCD_SEG7_SHIFT) | (((Digit[2] & 0x4) >> 2) << LCD_SEG16_SHIFT) ;
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT4_COM2, LCD_DIGIT4_COM2_SEG_MASK, data); /* 1Q 1K 1Col 1P  */
      data = (((Digit[2] & 0x8) >> 3) << LCD_SEG17_SHIFT);
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT4_COM2_1, LCD_DIGIT4_COM2_1_SEG_MASK, data); /* 1Q 1K 1Col 1P  */

      data = ((Digit[3] & 0x1) << LCD_SEG6_SHIFT) |(((Digit[3] & 0x2) >> 1) << LCD_SEG7_SHIFT) | (((Digit[3] & 0x4) >> 2) << LCD_SEG16_SHIFT) ;
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT4_COM3, LCD_DIGIT4_COM3_SEG_MASK, data) ; /* 1H 1J 1DP 1N  */
      data = (((Digit[3] & 0x8) >> 3) << LCD_SEG17_SHIFT);
      HAL_LCD_Write(&LCDHandle, LCD_DIGIT4_COM3_1, LCD_DIGIT4_COM3_1_SEG_MASK, data); /* 1H 1J 1DP 1N  */
#endif /* USE_STM32U083C_DISCO_REVB */
      break;
