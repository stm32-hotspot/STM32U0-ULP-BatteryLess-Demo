...
/**
  ...
  * @param  HCLK1Frequency HCLK1 frequency in Hz
                                         ...
                                         LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct)
{
  ...
  uint32_t pllrfreq = 0U, msi_range = 0U;
  ...
  if(UTILS_PLL_IsBusy() == SUCCESS)
  {
    ...
    if(status != ERROR)
    {
      ...
      pllrfreq = UTILS_GetPLLOutputFrequency(__LL_RCC_CALC_MSI_FREQ(LL_RCC_MSIRANGESEL_RUN,msi_range), UTILS_PLLInitStruct);
