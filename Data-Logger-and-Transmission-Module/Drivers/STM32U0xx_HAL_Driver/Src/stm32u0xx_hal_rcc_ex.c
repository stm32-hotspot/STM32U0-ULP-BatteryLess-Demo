...
void HAL_RCCEx_GetPeriphCLKConfig(RCC_PeriphCLKInitTypeDef  *PeriphClkInit)
{
  ...
  PeriphClkInit->PeriphClockSelection = RCC_PERIPHCLK_RTC  | RCC_PERIPHCLK_USART1 | RCC_PERIPHCLK_USART2 | RCC_PERIPHCLK_LPUART1 | RCC_PERIPHCLK_LPUART2 | \
                                        RCC_PERIPHCLK_I2C1 | RCC_PERIPHCLK_I2C3   | RCC_PERIPHCLK_LPTIM1 | RCC_PERIPHCLK_LPTIM2  |  RCC_PERIPHCLK_TIM1 | \
                                        RCC_PERIPHCLK_TIM15 | RCC_PERIPHCLK_RNG    | RCC_PERIPHCLK_ADC ;
...
{
  uint32_t PLLsource, PLLm, PLLn, PLLvco;
  ...
  if (PLLm != 0)
  {
    ...
    PLLvco = ((MSIRangeTable[(__HAL_RCC_GET_MSI_RANGE() >> 4U)] / PLLm )* PLLn);
      break;
    ...
    if(__HAL_RCC_GET_PLLCLKOUT_CONFIG(RCC_PLL_DIVP) != RESET)
    {
      PLL_Clocks->PLL_P_Frequency = (uint32_t)(PLLvco/(((RCC->PLLCFGR & RCC_PLLCFGR_PLLP) >>RCC_PLLCFGR_PLLP_Pos) + 1U));
    }
    else
    {
    ...
    if(__HAL_RCC_GET_PLLCLKOUT_CONFIG(RCC_PLL_DIVQ) != RESET)
    {
      PLL_Clocks->PLL_Q_Frequency = (uint32_t)(PLLvco/(((RCC->PLLCFGR & RCC_PLLCFGR_PLLQ) >>RCC_PLLCFGR_PLLQ_Pos) + 1U));
    }
    else
    {
    ...
    if(__HAL_RCC_GET_PLLCLKOUT_CONFIG(RCC_PLL_DIVR) != RESET)
    {
      PLL_Clocks->PLL_R_Frequency = (uint32_t)(PLLvco/(((RCC->PLLCFGR & RCC_PLLCFGR_PLLR) >>RCC_PLLCFGR_PLLR_Pos) + 1U));
    }
    else
    {
